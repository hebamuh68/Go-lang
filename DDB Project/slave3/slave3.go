...
func main() {
	...
	http.HandleFunc("/peptideSearch", func(w http.ResponseWriter, r *http.Request) {
		...
		fasta_file_path := "slave_3_search.fasta"
		json_file_path := "slave_3_searchfile.json"
	...
	// Listen on port 8088
	fmt.Println("Slave_3 starting on port 8093")
	if err := http.ListenAndServe("127.0.0.1:8093", nil); err != nil {
		log.Fatal(err)
	}
}
