...
func main() {
	...
	http.HandleFunc("/peptideSearch", func(w http.ResponseWriter, r *http.Request) {
		...
		fasta_file_path := "slave_4_search.fasta"
		json_file_path := "slave_4_searchfile.json"
	...
	// Listen on port 8088
	fmt.Println("Slave_4 starting on port 8094")
	if err := http.ListenAndServe("192.168.43.12:8094", nil); err != nil {
		log.Fatal(err)
	}
}
